...
vp rule
    (vp, logic: VP_logic, qstore: NP_qstore, agr:exists, gap:Gap, subcat: VP_subcat) ===>
    sem_head> (v, logic:V_logic, subcat: V_subcat),
...
s_gap rule
    ...
    cat> (np, logic:NP_Sub_logic, qstore: e_list, gap:None, NP),
    sem_head> (vp, logic:VP_logic, qstore: VP_qstore, gap:Gap, subcat:VP_subcat, VP),
    goal> resolve_gap_and_normalize(NP, VP, NP_Obj, S_logic, S_qstore).
...
apply_normalize_and_retrieve(LogicFunc, LogicArg, QStore, VP_subcat, NewLogic, NewQStore) if
    is_ambigous(VP_subcat),
    beta_normalize(@apply(LogicFunc, [LogicArg]), Norm_logic),
    retrieve(QStore, Norm_logic, NewQStore, NewLogic).

apply_normalize_and_retrieve(LogicFunc, LogicArg, QStore, VP_subcat, Norm_logic, QStore) if
    is_not_ambiguos(VP_subcat),
    is_empty(QStore),
    beta_normalize(@apply(LogicFunc, [LogicArg]), Norm_logic).
...
resolve_gap_and_normalize(
        (np, logic:NP_Sub_logic, NP), 
        (vp, logic: VP_logic, gap:Gap, subcat: VP_subcat), 
        (np, logic:NP_Obj_logic, qstore: NP_Obj_qstore, NP_obj), 
        S_logic, S_qstore
    ) if
    is_gap(Gap),
    append(VP_subcat, NP_Obj, VP_subcat),
    append(VP_subcat, NP, VP_subcat),
    beta_normalize(@apply(VP_logic, [NP_Obj_logic]), VP_Obj_logic),
    apply_normalize_and_retrieve(NP_Sub_logic, VP_Obj_logic, NP_Obj_qstore, VP_subcat, S_logic, S_qstore).
