...
vp rule
    (vp, logic: VP_logic, qstore: NP_qstore, agr:exists, subcat: [NP]) ===>
    sem_head> (v, logic:V_logic),
    cat> (np, logic:NP_logic, qstore: NP_qstore, NP),
...
s rule
    ...
    cat> (np, logic:NP_logic, qstore: e_list, agr:(quant, NP_agr), NP),
    sem_head> (vp, logic:VP_logic, qstore: VP_qstore, agr:NP_agr, VP),
    goal> append_np_and_process(
        NP, VP, VP_qstore, S_logic, S_qstore
    ).
